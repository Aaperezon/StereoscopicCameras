...
void box_filter_8u_c1(unsigned char* left_image, unsigned char* right_image, unsigned char* CPUoutput, const int width, const int height, const int widthStep, const int filterWidth, const int filterHeight)
{
    ...
    cout<<"box_filter_8u_c1: after malloc "<<endl;
...
void printHistogram(vector<unsigned char> left_image_vector){
  ...
  for(int i = 0; i < left_image_vector.size(); i+=4){
    ...
    histogram_g[left_image_vector[i+1]/histogram_steps]++;
    histogram_b[left_image_vector[i+2]/histogram_steps]++;
  }
  cout<<"RED"<<endl;
...
int main(int argc, char** argv) {
  ...
  for(int i = 0; i < left_image_vector.size(); i++) {
    ...
    out_image[i] = left_image_vector.at(i);
  } 
  ...
  */
  ...
  cout<<"after maximizing channels: ";
  ...
  cout<<"Size final "<<left_image_vector.size()<<endl;
  for(int i = 0; i < left_image_vector.size(); i++) {
    // if(i%4 == 3)
  ...
  cout<<"after augmenting and preparing vector to save: ";
  // printElapsedTime(time);
